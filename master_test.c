...
static bool wait_i2c_presence(uint8_t address, bool *online) {
  ...
  while (!time_reached(deadline)) {
    ...
    printf(
        "Received I2C response candidate: command=0x%02X, id=%u, value=%u\r\n",
        response.command, response.id, response.value);
    ...
    if (response.command != RSP_I2C_PRESENCE) {
      printf("ERROR: Expected I2C presence response 0x%02X, got 0x%02X\r\n",
             RSP_I2C_PRESENCE, response.command);
      continue;
    }
  ...
  printf("ERROR: Timeout waiting valid I2C presence response for address "
         "0x%02X\r\n",
         address);
  return false;
}
  ...
  while (!time_reached(deadline)) {
    ...
    if (response.command != RSP_SWITCH_STATE) {
      printf("ERROR: Expected switch state 0x%02X, got 0x%02X\r\n",
             RSP_SWITCH_STATE, response.command);
      continue;
    }
...
static bool wait_adc_value(uint8_t adc_id, uint16_t *value) {
  ...
  while (!time_reached(deadline)) {
    ...
    printf(
        "Received ADC response candidate: command=0x%02X, id=%u, value=%u\r\n",
        response.command, response.id, response.value);
    ...
    if (response.command != RSP_ADC_VALUE) {
      printf("ERROR: Expected ADC value response 0x%02X, got 0x%02X\r\n",
             RSP_ADC_VALUE, response.command);
      continue;
    }
...
static bool run_adc_test(void) {
  ...
  if (adc0_value < SLAVE_ADC0_MIN_EXPECTED ||
    ...
    printf("ERROR: ADC0 value %u out of expected range %u..%u\r\n", adc0_value,
           SLAVE_ADC0_MIN_EXPECTED, SLAVE_ADC0_MAX_EXPECTED);
    return false;
  }
  ...
  if (adc1_value < SLAVE_ADC1_MIN_EXPECTED ||
    ...
    printf("ERROR: ADC1 value %u out of expected range %u..%u\r\n", adc1_value,
           SLAVE_ADC1_MIN_EXPECTED, SLAVE_ADC1_MAX_EXPECTED);
    return false;
  }
  ...
  if (adc2_value < SLAVE_ADC2_MIN_EXPECTED ||
    ...
    printf("ERROR: ADC2 value %u out of expected range %u..%u\r\n", adc2_value,
           SLAVE_ADC2_MIN_EXPECTED, SLAVE_ADC2_MAX_EXPECTED);
    return false;
  }
...
static bool validate_adc_range(const char *name, uint16_t value,
...
static bool run_pwm_adc_test(void) {
  ...
  if (!validate_adc_range("PWM1 on master ADC0", pwm1_adc_value,
                          PWM1_ADC_MIN_EXPECTED, PWM1_ADC_MAX_EXPECTED)) {
    return false;
  }

  if (!validate_adc_range("PWM2 on master ADC1", pwm2_adc_value,
                          PWM2_ADC_MIN_EXPECTED, PWM2_ADC_MAX_EXPECTED)) {
    return false;
  }
